...
# 2020/06/30 首次發行 by A-Bo Lee abokuo@gmail.com
#
# 變數定義
...
cd /var/www/ #  for common linux system
...
sed -i "s/DB_HOST=127.0.0.1/DB_HOST=${db_server}/" /var/www/$site_name/.env
sed -i "s/DB_DATABASE=laravel/DB_DATABASE=${db_name}/" /var/www/$site_name/.env
sed -i "s/DB_USERNAME=root/DB_USERNAME=${db_username}/" /var/www/$site_name/.env
...
sed -i "s/APP_URL=http:\/\/localhost/APP_URL=https:\/\/${site_name}/" /var/www/$site_name/.env # for common linux system

# 定義資料庫編碼、校正以及不使用嚴謹模式
echo "It's important about the collation of your data in database, let it be utf8mb4_general_ci."
sed -i 's/utf8mb4_unicode_ci/utf8mb4_general_ci/' /var/www/$site_name/config/database.php # for common linux system
sed -i "s/'strict' => true/'strict' => false/" /var/www/$site_name/config/database.php # for common linux system
...
chown -R nginx:nginx /var/www/$site_name # for common linux system
...
echo "*  *  *  *  * root php /var/www/${site_name}/artisan schedule:run 1>> /dev/null 2>&1 " >> /etc/crontab
