...
# Web Server (Nginx), Database Connection (with MySQL Server), and FTP (VSFTPd), and AWStats website analytics tool.
# Before use this script, make sure you have files below:
# example.com.conf that located on /etc/nginx/, for Nginx virtual host configuration.
# example that localtd on /etc/vsftpd/vsftpd_user_conf/, for VSFTPd virtual user dirtctory setting.
# awstats.example.com.conf that located on /etc/awstats/, for AWStats website configuration.
# 2020 即時互動科技版權所有
# Changelog
...
sed -i "s/APP_URL=http:\/\/localhost/APP_URL=https:\/\/${site_name}/" /mnt/webdisk/$site_name/.env

# 定義資料庫編碼、校正以及不使用嚴謹模式
echo "It's important about the collation of your data in database, let it be utf8mb4_general_ci."
sed -i 's/utf8mb4_unicode_ci/utf8mb4_general_ci/' /mnt/webdisk/$site_name/config/database\.php
sed -i "s/'strict' => true/'strict' => false/" /mnt/webdisk/$site_name/config/database\.php
...
chown -R nginx:nginx /mnt/webdisk/$site_name
...
echo "*  *  *  *  * root php /mnt/webdisk/${site_name}/artisan schedule:run 1>> /dev/null 2>&1 " >> /etc/crontab
